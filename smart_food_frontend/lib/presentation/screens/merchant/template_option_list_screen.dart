...
class TemplateOptionListScreen extends StatefulWidget {
  final OptionGroupTemplateModel group;
  const TemplateOptionListScreen({super.key, required this.group});

  @override
  State<TemplateOptionListScreen> createState() =>
      _TemplateOptionListScreenState();
}

class _TemplateOptionListScreenState extends State<TemplateOptionListScreen> {
  @override
  void initState() {
    super.initState();
    Future.microtask(() {
      Provider.of<OptionTemplateProvider>(context, listen: false)
          .loadOptions();
    });
  }
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () {
          Navigator.pushNamed(
            context,
            AppRoutes.addTemplateOption,
            arguments: widget.group,
          );
        },
        child: const Icon(Icons.add, color: Colors.white),
      ),
      body: Consumer<OptionTemplateProvider>(
        builder: (context, provider, _) {
          final list = provider.options
              .where((o) => o.optionGroupTemplateId == widget.group.id)
              .toList();
  ...
  Widget _optionItem(OptionTemplateModel o) {
    return Container(
          ...
          PopupMenuButton<String>(
            ...
            onSelected: (val) async {
              if (val == "edit") {
                ...
                if (confirm == true) {
                  final ok = await Provider.of<OptionTemplateProvider>(
                    context,
                    listen: false,
                  ).deleteOption(o.id);
                  if (!mounted) return;
