...
import 'package:smart_food_frontend/data/services/reorder_service.dart';
import 'package:smart_food_frontend/data/services/api_client.dart';

class OrderService {
  static const String baseUrl = "http://10.0.2.2:8000/api/orders";
  ...
  static Future<OrderModel?> createOrder(Map<String, dynamic> body) async {
    final res = await ApiClient.send(
      (token) => http.post(
        Uri.parse("$baseUrl/"),
        headers: {
  ...
  static Future<bool> reorder(int orderId) => ReorderService.reorder(orderId);
}
